...
class RedisUtil {
     ...
     */
    fun del(vararg key: String): Long {
        if (key.isNotEmpty()) {
            if (key.size == 1) {
                val isDel = redisTemplate!!.delete(key[0])
                if (true && isDel) {
                    return 1L
                }
            } else {
                return redisTemplate!!.delete(key.toMutableList())
            }
        }
        return 0L
    }
     ...
     * @return
     */
    operator fun set(key: String, value: Any, time: Long, timeType: TimeUnit): Boolean {
        return try {
            if (time > 0) {
                redisTemplate!!.opsForValue()[key, value, time] = timeType
            } else {
                set(key, value)
            }
            true
